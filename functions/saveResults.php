<?php
	...
	if($_GET["action"]=="add"){
		...
		if(!file_exists($testRoot.$testResultsFilename)){
			$heading = '"chart_id","background_color","actual_color","picked_color","euclid_distance","hue_dist","saturation_dist","luma_dist","hue_delta","saturation_delta","luma_delta","time"';
			file_put_contents($testRoot.$testResultsFilename, $heading.PHP_EOL);
		}

		$row = '"'.$_POST["chart_id"].'","'.$_POST["bg_color"].'","'.$_POST["actual_color"].'","'.$_POST["picked_color"].'","'.$_POST["euclid_dist"].'","'.$_POST["hue_dist"].'","'.$_POST["saturation_dist"].'","'.$_POST["luma_dist"].'","'.$_POST["hue_delta"].'","'.$_POST["saturation_delta"].'","'.$_POST["luma_delta"].'","'.$_POST["time"].'"';
